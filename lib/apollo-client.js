...
async function apolloRequest(endpoint, apiKey, options = {}) {
  ...
  if (!response.ok) {
    ...
    if (isCreditError(errorText, response.status)) {
      ...
      // Don't throw - return a structured error instead
      // This allows graceful handling in bulk operations
      // Return a structured error that can be caught and handled gracefully
      // The error message starts with APOLLO_CREDITS_EXHAUSTED for easy detection
      throw new Error(`APOLLO_CREDITS_EXHAUSTED: Apollo.io credits exhausted. Enrichment will be skipped.`);
    }
...
async function enrichContact(supabase, contact) {
  try {
    ...
    // Check if this was a credit error - handle gracefully without logging
    if (error.message && (error.message.includes('APOLLO_CREDITS_EXHAUSTED') || isCreditError(error.message))) {
      return {
        success: false,
    ...
    // Only log non-credit errors (enrichment is optional)
...
async function bulkEnrichContacts(supabase, contacts) {
  try {
    ...
    for (let i = 0; i < contacts.length; i++) {
      ...
      try {
        // Check if Apollo is disabled before attempting enrichment
        if (isApolloDisabled()) {
        ...
        // If enrichment failed due to credits, mark all remaining as skipped and exit gracefully
        if (result.creditsExhausted || (result.error && isCreditError(result.error))) {
          // Silently skip - enrichment is optional, analysis will continue without it
          results.push({
            contact,
            success: false,
        ...
        // Only log errors that aren't credit exhaustion (enrichment is optional)
        if (!error.message || (!error.message.includes('APOLLO_CREDITS_EXHAUSTED') && !isCreditError(error.message))) {
          logError(`Error enriching contact ${i + 1}/${contacts.length}:`, error);
        }
        
        // Check if this was a credit error - stop bulk processing gracefully
        if (error.message && (error.message.includes('APOLLO_CREDITS_EXHAUSTED') || isCreditError(error.message))) {
    ...
    if (skipped > 0) {
      log(`Completed enrichment: ${succeeded} succeeded, ${failed} failed, ${skipped} skipped (credits exhausted)`);
    } else {
      log(`Completed enrichment: ${succeeded} succeeded, ${failed} failed`);
    }
    return results;
  } catch (error) {
