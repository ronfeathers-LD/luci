/**
 * Vercel Serverless Function for User-Role Management
 * 
 * Handles assigning and removing roles for users
 * Supports GET (list users with roles) and PUT (update user roles)
...
async function userHasRole(supabase, userId, roleName) {
  ...
  const { data, error } = await supabase
    ...
    `)
    .eq('user_id', userId)
    .limit(100); // Get all roles for the user, then filter client-side for case-insensitive match
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    logError('Error in user-roles function:', error);
