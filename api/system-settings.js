/**
 * Vercel Serverless Function for System Settings Management
 * 
 * Handles:
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      const { category, integrationStatus, integrationConfigs, type, userId } = req.query;
      ...
      // Update settings
      const { category, settingKey, value, userId } = req.body;

      if (!category || !settingKey || value === undefined) {
        return sendErrorResponse(res, new Error('Missing required fields: category, settingKey, and value'), 400);
      }
      ...
      const { action, type, config, userId } = req.body;

      if (action === 'updateIntegrationConfig') {
        // Update or create integration config
        if (!type || !config) {
          return sendErrorResponse(res, new Error('Missing required fields: type and config'), 400);
        }
        ...
        // Test integration connection
        if (!type) {
          return sendErrorResponse(res, new Error('Missing required field: type'), 400);
        }
        ...
        return sendSuccessResponse(res, testResult);
      } else {
        return sendErrorResponse(res, new Error('Invalid action. Use "updateIntegrationConfig" or "testIntegration"'), 400);
      }
    } else {
